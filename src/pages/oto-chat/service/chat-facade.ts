...
export class ChatFacadeService {
  ...
  initializeChat(): void {
    this.chatNavigationService.checkForOpenChatUser();
  }

  handlePendingChatUser(chatListComponent?: any): void {
    this.chatNavigationService.handlePendingChatUser(chatListComponent);
  }

  selectChat(chat: OtoChat): void {
    this.chatNavigationService.selectChat(chat);
  }

  selectFoundUser(userData: { nick: string; image: string }): void {
    this.chatNavigationService.selectFoundUser(userData);
  }

  openChatWithUser(userData: { nickName: string; image: string }): void {
    this.chatNavigationService.selectFoundUser({ 
  ...
  closeCurrentChat(): void {
    this.chatNavigationService.closeCurrentChat();
  }

  async sendMessage(content: string): Promise<void> {
    return this.messageStateService.sendMessage(content);
  }

  startEditMessage(message: OtoMessage): void {
    this.messageStateService.startEditMessage(message);
  }

  async completeEdit(messageId: string, content: string): Promise<void> {
    return this.messageStateService.completeEdit(messageId, content);
  }

  cancelEdit(): void {
    this.messageStateService.cancelEdit();
  }

  startDeleteMessage(message: OtoMessage): void {
    this.messageStateService.startDeleteMessage(message);
  }

  async confirmDelete(): Promise<void> {
    return this.messageStateService.confirmDelete();
  }

  closeDeleteModal(): void {
    this.messageStateService.closeDeleteModal();
  }

  setDeleteForBoth(value: boolean): void {
    this.messageStateService.setDeleteForBoth(value);
  }

  startReplyToMessage(message: OtoMessage): void {
    this.messageStateService.startReplyToMessage(message);
  }

  cancelReply(): void {
    this.messageStateService.cancelReply();
  }
  ...
  startChatWithUser(userData: { nick: string; image: string }): void {
    this.userSearchService.startChatWithUser(userData);
  }

  subscribeToUserDeletion(callback: (deletedUserInfo: UserDeletionInfo) => void) {
    return this.userStateService.subscribeToUserDeletion(callback);
  }

  subscribeToUserInfoUpdates(callback: (userInfo: UserUpdateInfo) => void) {
    return this.userStateService.subscribeToUserInfoUpdates(callback);
  }

  handleUserDeletion(deletedUserInfo: UserDeletionInfo) {
    return this.userStateService.handleUserDeletion(deletedUserInfo);
  }

  handleUserInfoUpdate(userInfo: UserUpdateInfo) {
    return this.userStateService.handleUserInfoUpdate(userInfo);
  }
  ...
  getCurrentChatState(): ChatState {
    return {
      selectedChat: this.userStateService.getSelectedChat(),
      selectedChatImage: this.userStateService.getSelectedChatImage(),
      selectedOtoChat: this.userStateService.getSelectedOtoChat(),
      currentUserNickName: this.userStateService.getCurrentUserNickName()
    };
  }

  getCurrentMessageState(): MessageState {
    return this.messageStateService.getCurrentMessageState();
  }

  getCurrentSearchState(): UserSearchState {
    return this.userSearchService.getCurrentSearchState();
  }


  isChatWithCurrentUser(chatNickName: string): boolean {
    return this.userStateService.isChatWithCurrentUser(chatNickName);
  }

  getDisplayChatName(chatNickName: string): string {
    return this.userStateService.getDisplayChatName(chatNickName);
  }
  ...
  isChatActive(chatNickName: string): boolean {
    return this.userStateService.isChatActive(chatNickName);
  }

  resetAllStates(): void {
    this.chatNavigationService.resetSelectedChat();
  }
